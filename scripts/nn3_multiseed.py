"""nn3(emb32) 10-seed 稳定化(仿 nn1)。出 nn3_10s submission + holdout R²。"""
...
def main():
    ...
    Ytr=torch.from_numpy(ytr).to(DEV); Wtr=torch.from_numpy(wtr).to(DEV); bs=16384; n_tr=len(Xt)
    hv=[]; tv=[]
    for i,sd in enumerate(SEEDS):
        ...
        print(f"seed {sd} ({i+1}/10): 增量 holdout={wr2(yv,np.mean(hv,0),wv):+.5f}",flush=True)
    ft=np.mean(tv,0); ft=np.where(np.isfinite(ft),ft,0.0)
    pd.DataFrame({"row_id":te["row_id"],"target":ft}).to_csv("/mnt/iscsi/hd/xxz/submissions/nn3_10s.csv",index=False)
    print(f"\nnn3(emb32) 10-seed holdout={wr2(yv,np.mean(hv,0),wv):+.5f} (单seed 0.00072)",flush=True)
